  ...
  m3_slice *  a_slice, *  b_slice, *  c_slice;
   ...
   //m3d_print(a_slice->row[0][0]);
    ...
    else if((c_slice->row[0][0].ncols ) <= M1RI_RADIX)
    {
      ...
      mul_64_m3d(x1.rows, a_slice->row[0][0].rows, b_slice->row[0][0].rows);

       mul_64_m3d(x2.rows, a_slice->row[0][1].rows, b_slice->row[1][0].rows);
	
       add_64_m3d(c_slice->row[0][0].rows, x1.rows, x2.rows) ;
        /*   */
		
 		 /*   */
      
        mul_64_m3d(x1.rows,  a_slice->row[0][0].rows, b_slice->row[0][1].rows );
        mul_64_m3d(x2.rows, a_slice->row[0][1].rows, b_slice->row[1][1].rows);
		

       add_64_m3d(c_slice->row[0][1].rows, x1.rows, x2.rows) ; 
	   /*   */
	 
       /*   */
       mul_64_m3d(x1.rows, a_slice->row[1][0].rows, b_slice->row[0][0].rows);
       mul_64_m3d(x2.rows, a_slice->row[1][1].rows, b_slice->row[1][0].rows);
      
       add_64_m3d(c_slice->row[1][0].rows, x1.rows, x2.rows); 
	   /*   */

	 // m3d_print(&c_slice->row[1][0]);
	   /*   */
       mul_64_m3d(x1.rows, a_slice->row[1][0].rows, b_slice->row[0][1].rows);
       mul_64_m3d(x2.rows, a_slice->row[1][1].rows, b_slice->row[1][1].rows); 

		 add_64_m3d(c_slice->row[1][1].rows, x1.rows, x2.rows); 
		  /*   */
		//m3d_print(&c_slice->row[1][1]);
		  /*   */
