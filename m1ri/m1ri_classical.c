  ...
  m5_slice  a_slice,  b_slice,  c_slice;
   ...
   m5d_quarter(&a_slice, a);
    ...
    if((c_slice.row[0][0].ncols) > cutoff)
    {
      ...
      m5d_t * x1 = m1ri_malloc(sizeof(m5d_t)); 
     m5d_t * x2 = m1ri_malloc(sizeof(m5d_t)); 
     ...
     m5d_create(x2, c_slice.row[0][0].nrows, c_slice.row[0][0].ncols);			    
       ...
       m5d_mul_naive_square(x2, &a_slice.row[0][1], &a_slice.row[1][0]);
    ...
    else if((c_slice.row[0][0].ncols ) <= cutoff)
    {
       ...
       mul_64_m5d(c_slice.row[0][0].rows, b_slice.row[0][0].rows, a_slice.row[0][0].rows);
        mul_64_m5d(c_slice.row[0][1].rows, b_slice.row[0][1].rows, a_slice.row[0][1].rows);
        mul_64_m5d(c_slice.row[1][0].rows, b_slice.row[1][0].rows, a_slice.row[1][0].rows);
         mul_64_m5d(c_slice.row[1][1].rows, b_slice.row[1][1].rows, a_slice.row[1][1].rows);
...
void m5d_classic_mul(m5d_t *c, m5d_t *a, m5d_t *b)
{
	if(a->ncols == b->nrows);
	{
		...
		lrows = a->nrows%64;
		lcols = b->ncols%64;  
		m5d_t * c_main_partition = malloc(sizeof(m5d_t));
		m5d_t * a_main_partition = malloc(sizeof(m5d_t));
		m5d_t * b_main_partition = malloc(sizeof(m5d_t));
		...
		m5d_mul_naive_square(c_main_partition, b_main_partition, a_main_partition); 
  ...
  m7_slice  a_slice,  b_slice,  c_slice;
   ...
   m7d_quarter(&a_slice, a);
    ...
    if((c_slice.row[0][0].ncols) > cutoff)
    {
      ...
      m7d_t * x1 = m1ri_malloc(sizeof(m7d_t)); 
     m7d_t * x2 = m1ri_malloc(sizeof(m7d_t)); 
     ...
     m7d_create(x2, c_slice.row[0][0].nrows, c_slice.row[0][0].ncols);			    
       ...
       m7d_mul_naive_square(x2, &a_slice.row[0][1], &a_slice.row[1][0]);
    ...
    else if((c_slice.row[0][0].ncols ) <= cutoff)
    {
       ...
       mul_64_m7d(c_slice.row[0][0].rows, b_slice.row[0][0].rows, a_slice.row[0][0].rows);
        mul_64_m7d(c_slice.row[0][1].rows, b_slice.row[0][1].rows, a_slice.row[0][1].rows);
        mul_64_m7d(c_slice.row[1][0].rows, b_slice.row[1][0].rows, a_slice.row[1][0].rows);
         mul_64_m7d(c_slice.row[1][1].rows, b_slice.row[1][1].rows, a_slice.row[1][1].rows);
...
void m7d_classic_mul(m7d_t *c, m7d_t *a, m7d_t *b)
{
	if(a->ncols == b->nrows);
	{
		...
		lrows = a->nrows%64;
		lcols = b->ncols%64;  
		m7d_t * c_main_partition = malloc(sizeof(m7d_t));
		m7d_t * a_main_partition = malloc(sizeof(m7d_t));
		m7d_t * b_main_partition = malloc(sizeof(m7d_t));
		...
		m7d_mul_naive_square(c_main_partition, b_main_partition, a_main_partition); 
