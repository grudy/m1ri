...
m3_slice  * a_slice, * b_slice,*  c_slice;
	...
	if((c->ncols) > (M1RI_RADIX << 1))
    {
       ...
       {
        x1 = m3d_sub( &a_slice->row[0][0], &a_slice->row[1][0]);  //1
        x2 = m3d_sub(&b_slice->row[1][1],&b_slice->row[0][1]);  //2
        m3d_qrt_mul(c_slice->row[2], x1, x2);  //3
        x1 = m3d_add(&a_slice->row[1][0],&a_slice->row[1][1]);  //4
        x2 = m3d_sub(&b_slice->row[0][1],&b_slice->row[0][0]);  //5
        m3d_qrt_mul(c_slice->row[3], x1, x2);    //6
        x1 = m3d_sub(x1,&a_slice->row[0][0]);//7
        x2 = m3d_sub(&b_slice->row[1][1],x2);  //8
        m3d_qrt_mul(c_slice->row[1],x1,x2); //9
        x1 = m3d_sub(&a_slice->row[0][1],x1);    //10
        m3d_qrt_mul(c_slice->row[0],x1,&b_slice->row[1][1]);   //11
        m3d_qrt_mul( x1 , &a_slice->row[1][1], &b_slice->row[1][1]);  //12
        c_slice->row[1] = m3d_add(x1 , c_slice->row[1]);   //13
        c_slice->row[2] = m3d_add(c_slice->row[1] , c_slice->row[2]);   //14
        c_slice->row[1] = m3d_add(c_slice->row[1] , c_slice->row[3]);   //15
        c_slice->row[3] = m3d_add(c_slice->row[2] , c_slice->row[3]);    //16
        c_slice->row[3] = m3d_add(c_slice->row[2] , c_slice->row[3]);  //17
        x2 = m3d_sub(x2, &b_slice->row[1][0]);            //18
        m3d_qrt_mul(c_slice->row[2], &a_slice->row[1][1], x2);            //19
        c_slice->row[2] = m3d_sub( c_slice->row[2], c_slice->row[0]);  //20
        m3d_qrt_mul(c_slice->row[0], &a_slice->row[0][1], &b_slice->row[1][0]);
        c_slice->row[0] = m3d_add(x1,c_slice->row[0] );
        m3d_free(x1);
    	m3d_free(x2);
       }	
    }
	...
	m3d_sub_64(x1->rows, a_slice->row[0]->rows, a_slice->row[2]->rows);  //1
        ...
        m3d_add_64(c_slice->row[2]->rows,c_slice->row[1]->rows , c_slice->row[2]->rows) ;   //14
        m3d_add_64(c_slice->row[1]->rows,c_slice->row[1]->rows , c_slice->row[3]->rows) ;   //15
...
static inline void m5d_qrt_mul(m5d_t * c,m5d_t *a, m5d_t * b )
{
    ...
    if((c_slice.row[0][0].ncols) > M1RI_RADIX)
    {
        x1 = m5d_sub(&a_slice.row[0][0], &a_slice.row[1][0]);  //1
        x2 = m5d_sub(&b_slice.row[1][1],&b_slice.row[0][1]);  //2
        m5d_qrt_mul(c_slice.row[2], x1, x2);  //3
        x1 = m5d_add(&a_slice.row[1][0],&a_slice.row[1][1]);  //4
        x2 = m5d_sub(&b_slice.row[0][1],&b_slice.row[0][0]);  //5
        m5d_qrt_mul(c_slice.row[3], x1, x2);    //6
        x1 = m5d_sub(x1,&a_slice.row[0][0]);//7
        x2 = m5d_sub(&b_slice.row[1][1],x2);  //8
    	m5d_qrt_mul(c_slice.row[1],x1,x2); //9
	m5d_sub_d(x1,&a_slice.row[0][1]);    //10  //todo is this the correct order????
        m5d_qrt_mul(c_slice.row[0],x1,&b_slice.row[1][1]);   //11
        m5d_qrt_mul(x1, &a_slice.row[1][1], &b_slice.row[1][1]);  //12
        c_slice.row[1] = m5d_add(x1 , c_slice.row[1]);   //13
        c_slice.row[2] = m5d_add(c_slice.row[1] , c_slice.row[2]);   //14
        c_slice.row[1] = m5d_add(c_slice.row[1] , c_slice.row[3]);   //15
        c_slice.row[3] = m5d_add(c_slice.row[2] , c_slice.row[3]);    //16
        c_slice.row[3] = m5d_add(c_slice.row[2] , c_slice.row[3]);  //15
        x2 = m5d_sub( x2, &b_slice.row[1][0]);            //18
        m5d_qrt_mul(c_slice.row[2], &a_slice.row[1][1], x2);            //19
        c_slice.row[2] = m5d_sub( c_slice.row[2], c_slice.row[0]);  //20
        m5d_qrt_mul(c_slice.row[0], &a_slice.row[0][1], &b_slice.row[1][0]);
        c_slice.row[0]  = m5d_add(x1,c_slice.row[0] );
	...
	m5d_sub_64(x1, &a_slice.row[0][0], &a_slice.row[1][0]);  //1
   	m5d_sub_64(x2,&b_slice.row[1][1],&b_slice.row[0][1]) ;  //2
        ...
        m5d_add_64(c_slice.row[1]->rows,x1->rows , c_slice.row[1]->rows) ;   //15
        ...
        m5d_add_64(c_slice.row[3]->rows,c_slice.row[2]->rows , c_slice.row[3]->rows) ;  //15
        m5d_sub_64(x2, x2, &b_slice.row[1][0]) ;            //18
        m5d_mul_64(c_slice.row[2]->rows, a_slice.row[3]->rows, x2->rows);            //19
        m5d_sub_64(c_slice.row[2], c_slice.row[2],c_slice.row[0]);  //20
...
static inline void m7d_qrt_mul(m7d_t * c,m7d_t *a, m7d_t * b )
{
    ...
    if((c_slice.row[0][0].ncols) > M1RI_RADIX)
    {
        ...
        x1 = m7d_sub( &a_slice.row[0][0], &a_slice.row[1][0]);  //1
        x2 = m7d_sub(&b_slice.row[1][1],&b_slice.row[0][1]);  //2
        m7d_qrt_mul(c_slice.row[2], x1, x2);  //5
        x1 = m7d_add(&a_slice.row[1][0],&a_slice.row[1][1]);  //4
        x2 = m7d_sub(&b_slice.row[0][1],&b_slice.row[0][0]);  //5
        m7d_qrt_mul(c_slice.row[3], x1, x2);    //6
        x1 = m7d_sub(x1,&a_slice.row[0][0]);//7
        x2 = m7d_sub(&b_slice.row[1][1],x2);  //8
        m7d_qrt_mul(c_slice.row[1],x1,x2); //9
        x1 = m7d_sub(&a_slice.row[0][1],x1);    //10
        m7d_qrt_mul(c_slice.row[0],x1,&b_slice.row[1][1]);   //11
        m7d_qrt_mul(x1, &a_slice.row[1][1], &b_slice.row[1][1]);  //12
        c_slice.row[1] = m7d_add(x1 , c_slice.row[1]);   //15
        c_slice.row[2] = m7d_add(c_slice.row[1] , c_slice.row[2]);   //14
        c_slice.row[1] = m7d_add(c_slice.row[1] , c_slice.row[3]);   //15
        c_slice.row[3] = m7d_add(c_slice.row[2] , c_slice.row[3]);    //16
        c_slice.row[3] = m7d_add(c_slice.row[2] , c_slice.row[3]);  //17
        x2  = m7d_sub( x2, &b_slice.row[1][0]);            //18
        m7d_qrt_mul(c_slice.row[2], &a_slice.row[1][1], x2);            //19
        c_slice.row[2] = m7d_sub( c_slice.row[2], c_slice.row[0]);  //20
        m7d_qrt_mul(c_slice.row[0], &a_slice.row[0][1], &b_slice.row[1][0]);
        c_slice.row[0] = m7d_add( x1,c_slice.row[0] );
	...
	m7d_sub_64(x1->rows, a_slice.row[0]->rows, a_slice.row[2]->rows);  //1
        ...
        m7d_add_64(c_slice.row[1]->rows,x1->rows , c_slice.row[1]->rows) ;   //15
...
*/
m3d_t * m3d_classic_mul(m3d_t *c, m3d_t  *a, m3d_t  *b)
{
	if (c == NULL)
	{
	c = m3d_create(a->nrows, b->ncols);
...
*/
m5d_t *  m5d_classic_mul(m5d_t *c, m5d_t  *a, m5d_t  *b)
{
...
m7d_t *  m7d_classic_mul(m7d_t *c, m7d_t *a, m7d_t *b)
{
